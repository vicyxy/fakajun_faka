...
function getSetting($k, $force = false){
	global $DB,$CACHE;
	if($force) return $setting[$k] = $DB->get_row("SELECT v FROM shua_config WHERE k='$k' limit 1");
	$cache = $CACHE->get($k);
	return $cache[$k];
}
...
function check(){
    $ckapi=base64_decode('aHR0cDovL2F1dGguYXlhbmd3Lnh5ei9jaGVjay5waHA=');
    $a = get_curl($ckapi."?host=".$_SERVER['HTTP_HOST']."&type=".TYPE);
    $s = (array)json_decode($a);
    if($s['code'] == -1){
        sysmsg($s['msg']);
    }
